...
describe("Constants - brazilianStocksMetadata", () => {
  it("should export brazilianStocksMetadata object", () => {
    expect(brazilianStocksMetadata).toBeDefined();
    expect(typeof brazilianStocksMetadata).toBe("object");
  });

  it("should have properties in brazilianStocksMetadata", () => {
    expect(Object.keys(brazilianStocksMetadata).length).toBeGreaterThan(0);
  });
});
...
describe("Constants - stocksPresets", () => {
  ...
  it("each preset should be either null or a function", () => {
    Object.values(stocksPresets).forEach((preset) => {
      expect(preset === null || typeof preset === "function").toBe(true);
    });
  });
